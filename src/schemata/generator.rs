...
impl SchemataGenerator {
    ...
    fn get_namespaces(&self, xsd_schema: &XsdSchema) -> Vec<SchemataNamespace> {
    ...
    fn get_schemas(&self, complex_types: &[XsdComplexType]) -> Vec<SchemataSchema> {
    ...
    fn get_fields(&self, elements: &[XsdElement]) -> Vec<SchemataField> {
        elements.iter().map(|e| {
            SchemataField {
                name: e.name.clone(),
                type_name: e.type_name.clone().unwrap_or_else(|| "string".to_string()),
                nullable: e.min_occurs.map(|m| m == 0).unwrap_or(false),
    ...
    fn get_enums(&self, simple_types: &[XsdSimpleType]) -> Vec<SchemataEnum> {
        simple_types.iter()
            .filter_map(|st| {
    ...
    fn get_annotations(&self, element: &XsdElement) -> String {
